// components/pages/sell/SellProductPage.tsx
...
function SellProductPage() {
  ...
  const handleSellThisProduct = () => {
    // Navegar a la página de edición con los datos del producto como base
    navigate('/sell/edit-product', { 
      state: { 
        baseProduct: product,
        isSellingExisting: true 
      } 
    });
  };

  const handleCreateSimilar = () => {
    // Navegar a crear un producto similar
          ...
          <nav className="flex mb-6" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-2 text-sm text-gray-600">
              ...
              <li className="flex items-center">
                ...
                <Link to="/sell" className="hover:text-blue-600">Vender</Link>
              </li>
              <li className="flex items-center">
                <span className="mx-2">/</span>
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="grid md:grid-cols-2 gap-8 p-6">
              ...
              <div className="space-y-6">
                ...
                {/* Selector de cantidad y acciones ESPECÍFICAS PARA VENDER */}
                <div className="space-y-4 pt-4 border-t border-gray-200">
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <h3 className="font-semibold text-yellow-800 mb-2">¿Quieres vender este producto?</h3>
                    <p className="text-yellow-700 text-sm">
                      Puedes vender este mismo producto o crear una versión similar con tus propias características.
                    </p>
                  </div>

                  <div className="flex space-x-4">
                    <button 
                      onClick={handleSellThisProduct}
                      className="flex-1 px-8 py-4 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center space-x-2"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                      </svg>
                      <span>Vender este Producto</span>
                    </button>
                    
                    <button 
                      onClick={handleCreateSimilar}
                      className="flex-1 px-8 py-4 border-2 border-blue-600 text-blue-600 font-semibold rounded-lg hover:bg-blue-50 transition-colors flex items-center justify-center space-x-2"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                      </svg>
                      <span>Crear Similar</span>
                    </button>
                  </div>
                </div>
